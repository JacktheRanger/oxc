...
pub struct MarkdownFormatContext<'a> {
    ...
    /// Per text of the current phrasing content (by start, sorted):
    /// `*` (bit 0) / `_` (bit 1) occurring unescaped up to and including it;
    /// an emphasis marker is not normalized to a character seen before it.
    literal_delimiters: RefCell<Vec<(u32, u8)>>,
}
...
impl<'a> MarkdownFormatContext<'a> {
    pub fn new(options: MarkdownFormatOptions, source_code: &'a str, blanks: &'a [Span]) -> Self {
        Self {
            ...
            literal_delimiters: RefCell::new(Vec::new()),
        }
    }
    ...
    pub fn literal_delimiters(&self) -> &RefCell<Vec<(u32, u8)>> {
        &self.literal_delimiters
    }
}
