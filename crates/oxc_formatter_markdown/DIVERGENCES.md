...
A normalization is skipped when a literal run before the node could pair with the normalized marker:
a code span keeps a longer fence when a literal backtick run of the shorter length precedes it,
`__strong__` keeps its underscores when a literal `*` precedes it, and `*emphasis*` keeps its asterisks
when a literal `_` precedes it (escaped runs do not count).
Prettier picks the shortest fence from the content alone and always normalizes markers,
so its output pairs with the stray run and moves the span.

Escapes follow the same reasoning: a `*` / `_` run inside emphasis that could pair is escaped character
by character (`\*\*\*`; Prettier's single backslash leaves `**`, which still pairs: `example-412.md`),
and a literal run right after a closing marker of the same character is escaped when the merged run could
open (`example-417.md`; the rule of three would otherwise unpair the closer).
The parser's own flanking rule (`oxc_markdown_parser::attention`) decides what pairs.
