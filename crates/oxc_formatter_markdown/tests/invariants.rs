...
// The default corpus (seed 0, 3000 documents) fails exactly on these documents, of the class below.
// Remove one when the class is fixed; any other failure is a regression.
// Other seeds sit around 0.2% of documents.
// - emphasis and strong nested in each other next to literal delimiter runs,
//   where changing a marker changes which runs pair (prettier/prettier#17353 family;
//   the parser's `attention` decides pairing, but the run lengths on both sides of a node are not re-simulated)
const KNOWN_FAILURES: &[(usize, ProseWrap)] =
    &[(1555, ProseWrap::Preserve), (1555, ProseWrap::Always), (1555, ProseWrap::Never)];
