            ...
            Block::CodeBlock(c) => {
                ...
                let info = match c.kind {
                    ...
                    CodeBlockKind::Fenced { info: Some(info), .. } => {
                        let raw = self.raw(info).trim();
                        let (lang, meta) =
                            raw.split_once(|c: char| c.is_ascii_whitespace()).unwrap_or((raw, ""));
                        let (lang, meta) =
                            (decode::decode(lang, true), decode::decode(meta.trim_start(), true));
                        format!("{lang} {meta}").trim_end().to_string()
                    }
                    _ => String::new(),
                };
